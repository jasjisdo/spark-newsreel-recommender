...
public class ConverterJson2IdomaarRelations {
		...
		try {
			...
			try {
				for (line = br.readLine(); line != null; line = br.readLine()) {
					...
					Object messageTypeJSON = jsonObj.get("event_type");
					if (messageTypeJSON != null && "recommendation_request".equalsIgnoreCase(messageTypeJSON+"")) {
	...
	public static void main(String[] args) {
		...
		// check whether argS[0] is a file or a directory
		...
		if (sourceFile.isFile()) {
			...
			// automatically process all files in the directory
			File[] childs = sourceFile.listFiles(new FilenameFilter() {
			...
			for (File file : childs) {
				converter(file.getAbsolutePath(), file.getAbsolutePath() + ".idomaar.txt" );
			}
		}
