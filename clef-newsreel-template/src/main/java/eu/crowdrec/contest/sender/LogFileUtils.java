	...
	public static String[] extractEvaluationRelevantDataFromInputLine(final String inputLine) {
		...
		try {
			...
			if (token.length > 4) {
				...
				try {
					deviceType = jsonObjectContextSimple.get("47").toString();
				} catch (Exception ignored) {
					System.err.println("[Exception] no deviceType found in " + tmpJsonObject);
				}
			
				String timestamp = "" + tmpJsonObject.get("timestamp");
				return new String[]{timestamp, userID+"", itemID+"", domainID+"", timestamp, deviceType};
			}
		} catch (Exception e) {
			e.printStackTrace();
			return null;
